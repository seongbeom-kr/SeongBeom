...
이것은 여전히 선형 함수입니다. 즉, h_2는 x에 대해 선형적입니다. 이를 일반화하면, L개의 선형 층을 쌓아도 결과는 여전히 선형 함수입니다:

$$
h_L = W_L \cdots W_2W_1x + \sum_{i=1}^{L} W_i b_{i-1}
$$

여기서 b_0 = x입니다. 따라서 다층 선형 모델은 본질적으로 단일 선형 모델과 동일한 표현력을 가집니다.

반면 비선형 활성화 함수  \phi 를 사용하면 다음과 같이 각 층에서 비선형성이 도입됩니다:
