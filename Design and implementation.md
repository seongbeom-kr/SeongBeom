...
형상관리
- 소프트웨어 개발 과정에서 생성되는 소프트웨어 엘리먼트들에 대한 변경을 체계적으로 관리함으로써 엘리먼트간의 통일성을 유지하고 변경 과정의 추적을 용이하게 하는 프로세스
- 여러 버전을 관리하는 프로세스
...
- 오픈 소스 비즈니스 : 판매가 아닌, 지원을 통해 이익을 얻음(커뮤니티 확대 추구)

## licensig
자유롭게 사용가능하지만, 아무나 그들이 원하는대로사용할 수 있는건 아님
- 제약사항 확인
...
Berkley Standard Distribution : 오픈소스를 이용하여 개발한 경우 변경된 부분에 대해 리퍼블리쉬 할 의무 없음, 판매도 가능
하지만 원작 코드는 인정해야함
...
양방향 연관 : 두 클래스가 같은 패키지 안에 있어야함, 양방향은 서로 상대편 객체를 알고 있어야 하므로 서로를 알기 위한 퍼블릭 메소드와 프라이빗 변수가 존재햐아함

### 연관의 역할
- 역할의 추가 : 각 클래스가 다른 클래스에 대하여 알고 있어야 하는 경우
...
## 순서 다이어그램의 구현
코딩 방법 
1. 메시지는 메소드 호출로 코딩, 객체 생성은 생성자 호출
2. 메시지 받는 객체의 클래스안에 메소드 구현
3. 분기 구조는 if-else 문장과 같은 조건문으로 구현
4. 병렬구조는 스레드로 구현

## 상태 다이어그램의 구현
- 상태다이어그램을 클래스로 매핑
- 상태 정보를 저장하기 위한 속성 추가
- 이벤트 -> 메소드 구현, 변화나 액션은 메소드 안에 탑재

## 액티비티 다이어그램의 구현
액티비티나 프로시저 안에서 수행되어야할 액션들의 순서를 나타냄
- 제어 객체나 서브 시스템의 알고리즘이나 제어 흐름을 나타냄
- 프로그램의 위치, 즉 프로그램의 제어문, 반복문으로 액티비티 다이어그램의 제어흐름을 구현
...
## 데이터 변환과 시스템 전환 정책
데이터 변환
- 사용하는 시스템의 데이터는 새 시스템이 도입되면서 전환되어야 함
