...
>[!note]  
>정규화의 정의가 포괄적인 만큼 정규화 기법도 다양하지만, 기본적인 접근 방법은 다음과 같이 몇가지로 정리할 수 있음
...
미니배치 단위로 정규화하므로 미니배치의 구성에 따라 데이터의 분포가 변형되어 확률적 성질이 생기고 그에 따라 모델의 성능이 높아진다.

최근 연구에 따르면 공변량변화를 없애기보단 손실과 그라디언트 변화를 제야하여 곡면을 부드럽게 만들어줌으로써 모델의 학습 성능이 향상한다는 것이 증명되었다.

# 4. 가중치 감소
> _가중치 감소(weight decay)_ 는 학습 과정에서 작은 크기의 가중치를 찾게 만드는 정규화 기법

## 4.1 가중치 감소 적용 방식
가중치 감소는 가중치의 크기를 제한하는 제약조건으로서 손실함수의 일부항으로 표현할 수 있다.

$$J(w) = J(w) + \lambda R(w)$$ 람다가 커질 수록 정규화 항의 비중이 커지면서 가중치 크기는 작아지고, 람다가 작을수록 정규화 항의 비중이 작아지면서 가중치 크기는 커진다. 

따라서 람다에 따라 가중치의 개수가 달라질 수 있어서 람다는 모델의 복잡도를 조정하는 역할을 한다고 볼 수 있다.

- 릿지 회귀 (L2) 
- 라소 회귀 (L1)

## 4.3 정규화 효과
유효 파라미터 수가 줄어들고 작은 모델이 되어 성능이 빨라짐

# 5. 조기종료(Ealry Stopping)
> [!note]+
> 조기 종료는 모델이 과적합되기 전에 훈련을 멈추는 기법이다.
> 
> 훈련하는 동안 주기적으로 성능 검증을 하다가 성능이 더 좋아지지 않으면 과적합이 되었다고 판단하고 훈련을 멈춘다.
> 
> 보통 epoch단위로 성능검증을 하며 epoch보다 자주 검증할 때는 배치 실행 단위로 검증

## 5.1 조기 종료 기준
조심해야할 점은 모델의 성능이 향상하지 않더라도 바로 종료해서는 안된다. 즉 지속적인 성능의 정체 또는 하락이 판단되면 그때 종료하는 것이 좋다.

또한 어느 기준으로 종료할 건지도 정해야한다. 보통은 모델 오차를 기준으로 종료하지만, 정확도와 같은 다른 성능 척도를 사용할 수 있다.

# 6. 데이터 증강
>[!note]+
>훈련데이터의 양을 늘려 과적합을 막는 기법
>
>데이터를 만드는건 비용이 많이 드니 쉽게 데이터를 늘리는 방법

## 6.1 데이터 증강 기법
가장 기본적인 방법은 훈련데이터를 조금씩 변형해서 새로운 데이터를 만드는 방법이다.

좀 더 고도화된 방법으로는 훈련데이터의 분포를 학습해서 생성모델을 만든 뒤에 새로운 데이터를 생성하는 방법이 있다.

### 데이터 증강을 어떤 방식으로 실행해야 할까?
미리 증강해서 사용하는 방법도 존재하지만, 데이터를 확률적으로 변형하면, 무한히 많은 변형이 생기므로 일반적으로는 훈련과정에서 실시간으로 데이터를 증강한다.

## 6.2 클래스 불변 가정
데이터 증강을 할 때는 클래스 불변 가정을 따라야 한다. 클래스 불변 가정은 데이터를 증강할 때 클래스가 바뀌지 않도록 해야한다는 것이다.

## 6.3 데이터 증강 방식 선택
데이터 종류와 문제에 따라 다르다. 

이미지 이동, 회전, 좌우 대칭, 왜곡, 잡음 추가, 색깔 변환 등 사용할 수 있다.

요즘은 자동 데이터 증강 기법들이 연구되고 있다.
