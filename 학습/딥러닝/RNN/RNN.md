...
![](https://i.imgur.com/iVls52L.png)


- 과거 정보를 잊기 위한 단계이다.
- 이 단계에서는 $h_{t-1}$과k $x_t$를 받아서 시그모이드 레이어를 통해 0과 1 사이의 값을$C_{t-1}$에 보내준다.
- 그 값이 1이면 이전 상태의 정보를 저장하고, 아니면 버리는게 된다.
...
##### 그레디언트 소실이 생기지 않는 이유
셀 상태를 연결하는 경로에서 왜 그래디언트 소실이 발생하지 않을까?
