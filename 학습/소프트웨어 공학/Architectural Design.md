...
- 계층적 아키텍쳐로 시스템을 구조화
- 대개의 경우 트랜잭션 기반의 시스템
	- 정보시스템의 경우 데이터베이스 트랜잭션을 포함하기 때문
- 레이어는 유저 인터페이스, 유저 커뮤니케이션, 정보 탐색, 시스템 데이터베이스 포함함

![](https://i.imgur.com/siWxCVa.png)

![](https://i.imgur.com/mi1q2hU.png)

#### 웹 베이스 정보시스템
오늘날 정보 및 자원 관리 시스템은 웹 기반 작성

멀티 티어 클라이언트 서버 아키텍쳐로 구현됨
- 웹서버 : 웹 브라우저 상의 사용자 인터페이스를 이용하여 사용자와의 의사소통을 담당
- 어플리케이션 서버 : 어플리케이션 특정 로직, 정보 저장소, 탐색 리퀘스트 등을 구현
- 데이터베이스 서버 : 정보를 데이터베이스로 이동시키고 트랜잭션을 관리함

### [3] 언어 처리 시스템
- 자연언어, 혹은 인공언어를 받아들인 후, 이를 다른 형태의 언어로 생성하는 시스템
- 인터프리터를 포함하는 경우 언어에 명시된 내용을 실제 수행할 수도 있음

![](https://i.imgur.com/9xKDrkp.png)
#### 컴파일러 컴포넌트

![](https://i.imgur.com/1wXzGWT.png)
