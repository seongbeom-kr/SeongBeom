...
release testing : 분리된 테스트 팀이 시스템을 고객에게 릴리즈하기 전에 완성된 버전을 테스트
user testing : 유저가 자신의 환경에서 시스템 테스트

### 개발 테스트
개발 테스트는 시스템 개발팀에 의해 수행되는 모든 테스트 활동을 포함
- 유닛 테스트 : 개별 프로그램단위, 객체 클래스들을 테스트함 -> 오브젝트나 메소드 기능 테스트 위주
- 컴포넌트 테스트 : 다수의 개별단위들 통합 후 인터페이스 테스트 위주
- 시스템 테스트 : 시스템 구성 컴포넌트들 통합, 하나의 시스템으로 만든 후 테스트, 상호작용 테스트 위주

#### 유닛 테스트
- 결함발견
- 메소드, 속성들을 가지는 객체 클래스
	- 임의의 클래스에 대한 테스트 coverage
		- 오브젝트에 있는 모든 계산들 수행
		- 오브젝트 모든 속성을 심문 준비
		- 가능한 모든 상태들을 실행
	- 상속은 오브젝트 클래스 테스트 과정을 어렵게 만들수 있음
##### 테스트 전략
부분 테스트 : 입력을 동일 특성을 지닌 그룹으로 나눔 , 테스트는 각 그룹을 대표할 수 있도록 설계
가이드라인 기반 테스트 : 프로그래머가 주로 생성하는 에러를 반영하고 있음, 

##### 부분 테스트
- 입력, 출력 데이터는 다수의 클래스들로 나누어질 수 있음
- 각 클래스들은 동일한 부분으로 볼 수 있으며, 각 클래스에 속한 멤버들은 같은 행동을 보임
##### 테스트 가이드라인
가이드라인
	시퀀스 값이 하나만 가지고 있는 경우, 다양한 크기의 시퀀스, 시퀀스 부분집합으로 시행, 길이가 0인것도 확인

일반적인 테스트 가이드라인
	모든 에러메시지를 다 생성할수있도록 입력값 지정할 것
	같은 입력을 반복하여 넣던가, 입력 시리즈를 넣어볼 것
	타당하지않는 아웃풋이 생성되는 입력 고의로 넣기
	매우 크거나 작은 결과값 도출 의도


##### 페스 테스트
목적 : 논리적으로 모든 경로를 테스트
과정 : 플로그 그래프를 그려 의사결정과정과 제어흐름을 표현함 -> 모든 노드를 작성
	
#### 컴포넌트 테스트
소프트웨어 컴포넌트 : 서로 상호작용하는 다수의 객체들의 집합
테스트 : 컴포넌트 인터페이스가 명세와 부합하여 작동됨을 보임, 이때 각 객체에 대한 유닛 테스트는 수행 완료된 상태임

##### 인터페이스 테스트
목적 : 인터페이스에 대한 잘못된 가정으로 인해 발생할 수 있는 결함발견
인터페이스 타입 
- 파라미터 인터페이스 
- 공유메모리
- 프로시절
...
##### 인터페이스 에러
인터페이스 사용안함
- 호출 컴포넌트가 피호출 컴포넌트의 인터페이스를 잘못사용하여 에러발생하는 경우
인터페이스 잘못이해하는 경우
- 호출 컴포넌트가 피호출 컴포넌트의 행동양식에 대해 잘못 이해하고 있는 경우
타미잉 에러
- 호출 및 피호출 컴포넌트가 상이한 속도로 작동하여 벗어난 정보에 접근되는 경우
##### 인터페이스 테스트 가이드라인
피호출 프로시져의 파라미터들은 범위의 극대극소값을 전달하여 테스트 함
포인터 파라미터 -> null
컴포넌트가 fail하는 경우 테스트
메시지 패싱의 경우 스트레스 테스트 수행
공유메모리 -> 컴포넌트가 활성화되는 순서를 다양하게 지정하여 테스트

#### 시스템 테스트
- 개발 단계 동안 시스템 테스트
	- 시스템 생성을 위한 컴포넌트 통함 후 테스트
	- 재사용 컴포넌트들도 통합하여 완전한 시스템을 만든 후 테스트
	- 시스템 테스트는 개별과정이 아닌 집합과정
	- 독립된 팀에서 수행
- 체크사항
	- 컴포넌트 호환가능?
	- 통합 행동을 살펴볼 것
##### 유스케이스 테스트
- 시스템 상호작용을 식별하기 위해 작성된 유스케이스들은 시스템 테스팅을 위한 기본자료로 사용될 수 있음
- 각 유스케이스는 대개의 경우 다수의 시스템 컴포넌트에 대한 테스팅을 포함함
- 시퀀스 다이어그램을 통해 테스트할 컴포넌트 및 상호작용을 알아낼 수 있음
